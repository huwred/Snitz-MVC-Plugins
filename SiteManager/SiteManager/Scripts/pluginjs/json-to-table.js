...
function ConvertJsonToTable(parsedJson, tableId, tableClassName, linkText)
{
    ...
    var italic = '<i>{0}</i>';
    var link = '<a href="{0}/?col={1}" class="row-edit">{2}</a>';
    var dellink = '<a href="{0}/?col={1}" class="del-col">{2}</a>';
    //Pattern for table                          
            ...
            if(headers)
            {
                ...
                for (var i = 0; i < parsedJson.length; i++)
                {
                    for (var j = 0; j < headers.length; j++)
                    {
                        ...
                        var isUrl = false; //urlRegExp.test(value) || javascriptRegExp.test(value);
                    ...
                    if (linkText) {
                        tbCon += tdRow.format(link.format(linkText, recKey, '<i class="fa fa-pencil fa-1_5x"></i>') + ' ' + dellink.format(linkText.replace(/edittable/gi, "DelColumn"), recKey + '&action=del', '<i class="fa fa-trash fa-1_5x"></i>'));
                        //tbCon += tdRow.format(dellink.format(linkText.replace(/edittable/gi,"DelColumn"), recKey + '&action=del', '<i class="fa fa-trash"></i>'));
                    }
                    trCon += tr.format(tbCon);
                    tbCon = '';
                }
