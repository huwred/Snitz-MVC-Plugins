...
namespace PhotoAlbum.Models
{
    public class PhotoRepository : IDisposable
    {
        ...
        public PhotoRepository(int memberid=0, int pagesize = 30)
        {
            _memberid = memberid;
            Pagesize = pagesize;
        ...
        public IEnumerable<int> PhotoIdList()
        {
            using (var context = new SnitzDataContext())
            {
                return context.Query<int>("SELECT I_ID FROM FORUM_IMAGES WHERE I_PRIVATE=0 AND I_NOTFEATURED=0");
            }
        ...
        public List<AlbumGroup> AlbumGroups()
        {
            using (var context = new SnitzDataContext())
            {
                var res = context.Fetch<AlbumGroup>("ORDER BY O_GROUP_ORDER");
                return res;
            }
        } 
        public List<AlbumList> AlbumIndex(int pagenum)
        {
            ...
            using (var context = new SnitzDataContext())
            {
                ...
                data = context.Fetch<AlbumList>(sql).Skip((pagenum-1)*Pagesize).Take(Pagesize).ToList();
